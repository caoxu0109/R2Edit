...
def summarize(
    ...
    #for run_dir in [Path(abs_path)]:
        ...
        for case_file in files:
            ...
            forget_acc = data['step1_post']['orignal_correct']
            relationscc = data['step1_post']['rewrite_prompts_correct']+data['step2_post']['rewrite_prompts_correct']
            cur_sum["relation_acc"].append(np.mean(relationscc))
            cur_sum["forget_acc"].append(np.mean(forget_acc))
            for prefix in ["pre","post", "step1_post", "step2_post"]:
        ...
        for prefix in ["pre", "step1_post", "step2_post"]:
