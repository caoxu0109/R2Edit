...
from dsets import (
    ...
    RelaDataset,
    RelaTwoStepDataset,
    RelaThirdStepDataset,
    RelationEditDataset,
)
from experiments.py.eval_utils_counterfact import compute_rewrite_quality_counterfact
from experiments.py.eval_utils_zsre import compute_rewrite_quality_zsre
from experiments.py.eval_relation_zsre import compute_forget_quality_zsre
...
DS_DICT = {
    ...
    "zsre_relation": (RelaDataset, compute_forget_quality_zsre),
    "zsre_twostep": (RelaTwoStepDataset, compute_rewrite_quality_zsre),
    "zsre_thridstep": (RelaThirdStepDataset, compute_rewrite_quality_zsre),
    "relation_final": (RelationEditDataset, compute_rewrite_quality_zsre),
}
...
def main(
    ...
    for record_chunks in chunks(ds, num_edits):
        # first_chunk = [item[0] for item in record_chunks]
        # second_chunk = [item[1] for item in record_chunks]
        # thrid_chunk = [item[2] for item in record_chunks]
        first_chunk = []
        second_chunk = []
        third_chunk = []
        for item in record_chunks:
            if len(item) == 1:
                first_chunk.append(item[0])
            elif len(item) == 2:
                first_chunk.append(item[0])
                second_chunk.append(item[1])
            elif len(item) == 3:
                first_chunk.append(item[0])
                second_chunk.append(item[1])
                third_chunk.append(item[2])
            else:
                raise ValueError(f"Unexpected number of items in record: {len(item)}. Expected 1, 2, or 3.")
        ...
        case_ids = [record["case_id"] for record in first_chunk]
        ...
        if any(alg in alg_name for alg in ["AlphaEdit", "MEMIT_seq", "NSE"]):
            ...
            print("Edited step1 finished")
            edited_model, cache_c ,_= apply_algo(
                model,
                tok,
                [
                    {"case_id": record["case_id"], **rewrite_dict}
                    for record in second_chunk
                    for rewrite_dict in (
                        record["requested_rewrite"]
                        if isinstance(record["requested_rewrite"], list)
                        else [record["requested_rewrite"]]
            ...
            print("Edited step2 finished")
            edited_model, cache_c ,_= apply_algo(
                model,
                tok,
                [
                    {"case_id": record["case_id"], **rewrite_dict}
                    for record in third_chunk
                    for rewrite_dict in (
                        record["requested_rewrite"]
                        if isinstance(record["requested_rewrite"], list)
                        else [record["requested_rewrite"]]
    ...
    for record in ds:
        if len(record) == 1:
            ...
            metrics = {
                ...
                "step3_rewrite": step3["requested_rewrite"],
                "step3_post": ds_eval_method(
                    model,#EDITED MODEL
                    tok,
                    step3,
                    *(
                        gen_test_vars
                        if step3["case_id"] % generation_test_interval == 0
                        else [None, None]
                    ),  # Only test generation every generation_test_interval cases
                ),
            }
        else:
            raise ValueError(f"Unexpected number of items in record: {len(record)}. Expected 1, 2, or 3.")
        out_file = Path(case_result_template.format(num_edits, step1["case_id"]))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ds_name",
        choices=["mcf", "cf", "zsre", "mquake","zsre_relation","zsre_twostep","zsre_thridstep","relation_final"],
        default="zsre",
